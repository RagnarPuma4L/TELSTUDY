...
def reproducir_sonido_con_notificacion(nombre_archivo, titulo, mensaje):
    def play():
        telmi_icon = os.path.join(os.path.abspath("."), "assets", "telmifr.png")
        ruta = os.path.join(os.path.abspath("."), "assets", "sounds", nombre_archivo)
        if os.path.exists(ruta):
            sonido = pygame.mixer.Sound(ruta)
            sonido.play()
                
            # notificación
            toast = Notification(app_id="Telematics Study Tool", title=titulo, msg=mensaje, icon=telmi_icon)
            toast.show()
...
def main(page: ft.Page):
    ...
    def change_maps(e):
        ...
        #Función para los containers de las instrucciones (qué latero escribir cada uno)
        def cont_instruccion(texto):
            return ft.Container(
                content=ft.Text(texto, size=14, color="#E0E7FF"),
                padding=12,
                bgcolor="#1E40AF",
                border=ft.border.all(1, "#818CF8"),
                border_radius=12,
                shadow=ft.BoxShadow(blur_radius=6, color="#6466F1", offset=ft.Offset(0,3))
            )
         #Instrucciones
        instrucciones = [
            "Click Izquierdo: Dibujar",
            "Mantén Ctrl + Z: Borrar lentamente",
            "Ctrl + X: Limpiar todo"
        ]
        ...
        piza_card = ft.Card(
            content=ft.Container(
                content=ft.Column([
                    ft.Row([ft.Text("PIZARRA", size=30, color=blanco, weight=ft.FontWeight.W_700)], alignment="center"),
                    ft.Divider(height=20, color="#1E40AF"),
                    ft.Row([
                        ft.Column([
                            ft.Text("Instrucciones", size=22, weight=ft.FontWeight.W_600, color=blanco),
                            *[cont_instruccion(texto) for texto in instrucciones],
                            ft.Divider(height=20, color="#1E40AF"),
                            ft.ElevatedButton(
                                text="Abrir Pizarra",
                                icon=ft.Icons.DRAW,
                        ...
                        #Imagen derecha (en process)
                        ft.Container(
                            content=ft.Image(src="telmifr.png", width=200, height=200),
                            padding=20, bgcolor="#1E3A8A", border_radius=20,
                            shadow=ft.BoxShadow(blur_radius=10, color="#4F46E5", offset=ft.Offset(0,4))
                        )
                    ], spacing=30)
                ], spacing=10),
                padding=24,
        ...
        page.add(piza_card, navigation_bar)
        page.update()
    ...
    # Pomodoro Simple
                ...
                if display == timer_display:
                    ...
                    while (work_hour > 0 or work_min > 0 or work_sec > 0) and temporizador == True:        
                        ...
                        if (work_hour > 0 and work_min == 0) and stop == False: #0:59
                            ...
                            work_sec = 59
                            timer_display.value = formato_tiempo(work_hour,work_min,work_sec)
                        if work_hour == 0 and work_min == 0 and work_sec ==0:
                            reproducir_sonido_con_notificacion("tada.wav", titulo="¡Estudio Terminado!", mensaje="Ahora toma un descanso. o(￣▽￣)ｄ")
                            temporizador = False
                ...
                if display == timer_display2:
                    ...
                    while ((rest_hour > 0 or rest_min > 0 or rest_sec > 0)) and temporizador2 == True:
                        ...
                        if (rest_hour > 0 and rest_min == 0)and stop == False: #0:59
                            ...
                            rest_sec = 59
                            timer_display2.value = formato_tiempo(rest_hour,rest_min,rest_sec)
                        if rest_hour == 0 and rest_min == 0 and rest_sec == 0:
                            reproducir_sonido_con_notificacion("tada.wav", titulo="Descanso Terminado.", mensaje="¡Inicia otro tiempo de estudio y sigue estudiando! (*^▽^*)")
                            temporizador2 = False
        ...
        threading.Thread(target=lambda: actualizar_displays(timer_display, work_hour, work_min, work_sec, timer_display2, rest_hour, rest_min, rest_sec), daemon=True).start()
